...
class KNN:
    ...
    def fit(self, data, unknowns):
        ...
        if len(data) >= self.k:
            raise ValueError('K is less than total voting groups. {} >= {}'.format(
                             (len(data)), self.k))
    ...
    def calc_distances(self):
        ...
        for group in self.data:
            for features in self.data[group]:
                ...
                distance = np.linalg.norm(np.array(features)-np.array(self.unknowns))
...
if __name__ == '__main__':
    ...
    # Import data
    df = pd.read_csv('..\\data_sets\\breast-cancer-wisconsin.data', header=None)
